<?php
$excel_filename = $_GET[filename] . "_" . date(Y.m.d);
...
4)how to reuse this code to create a dump for ANY MySQL table on your server:
  ...
  c)all of the above also holds true for the name of the Database:
	...
	comment out //$database = "your database"; in this script below 
	and then link to this file like:
	"this_file_name.php?$database=your_database&DB_TBLName=your_table_name..."
...
$table_seat = "dictionary";
//$table_seat,  $database, may also be commented out & passed to the browser
...
$title = "Dump For Table $table_seat from Database $database on $now_date";
/*
...
$fn = $excel_filename . "." . $file_ending;
//echo "here comes fn ...<br>";
echo $fn;
//break;
...
*/
    while($row = mysqli_fetch_row($result))
    {
		...
		$schema_insert = preg_replace("/\r\n|\n\r|\n|\r/", " ", html_entity_decode($schema_insert));
        $schema_insert .= "\t";
        print(trim($schema_insert));
        print "\n";
    }
